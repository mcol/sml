...
This is the doxygen documentation for the Structured Modelling Language (SML)
Interface to OOPS.

SML is a structured modelling extension to AMPL. It is designed as a
preprocessor to AMPL. The driver will read the SML file, analyse the
intended problem/matrix structure, create a separate (plain) AMPL file
for each node in the matrix tree and processes them through AMPL
(therefore creating *.nl files), and fill internal data structures
with information about the resulting matrix tree.
...
In the Makefile the following lines need to be set to te correct values
\code
OOPSHOME=
AMPLSOLVER=
\endcode
...
In order to preocess an SML file say
...
- sml < name-of-smlfile (which probably results in an array)
- ampl script.scr
...
ampl.l is the LEX/FLEX input file. It mainly consists of a list of tokens that should be recongnised in SML.
...
tasks. Also there is the possibility to recursivelt 'tag' all
...
instance of an AmplModel object). The indexing expresssion that
...
ExpandedModel objects (with their corresponding NlFile objects): One
specifying the row node, i.e. the NlFile from which the constraints
should be taken, the other one specifying the col node, i.e. which
variables should be taken. These are the two parameters in the
OOPSBlock constructor.
...
\attention This part of the implementation needs a lot of comparing
strings against strings in nested loops and is quite inefficient.

This is all done in the costructor 
